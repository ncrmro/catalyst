...
var _ = Describe("Environment Controller", func() {
	...
	Context("When reconciling a Helm environment", func() {
		...
		It("should migrate legacy Source field to Sources array", func() {
			// Create project
			migrationProject := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
			...
			_ = k8sClient.Delete(ctx, migrationProject)
			Expect(k8sClient.Create(ctx, migrationProject)).To(Succeed())

			// Create environment with legacy Source field (not Sources)
			legacySource := catalystv1alpha1.EnvironmentSource{
				Name:      "main",
				CommitSha: "legacy123",
				Branch:    "main",
			}
			migrationEnv := &catalystv1alpha1.Environment{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "migration-env",
					Namespace: namespace,
				},
				Spec: catalystv1alpha1.EnvironmentSpec{
					ProjectRef: catalystv1alpha1.ProjectReference{Name: "migration-project"},
					Type:       "helm-env",
					Source:     &legacySource,                          // Using legacy field
					Sources:    []catalystv1alpha1.EnvironmentSource{}, // Empty
				},
			}
			_ = k8sClient.Delete(ctx, migrationEnv)
			Expect(k8sClient.Create(ctx, migrationEnv)).To(Succeed())
			...
			// Reconcile should migrate the field
			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{
				NamespacedName: types.NamespacedName{Name: "migration-env", Namespace: namespace},
			})
			Expect(err).NotTo(HaveOccurred())

			// Verify migration happened
			env := &catalystv1alpha1.Environment{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "migration-env", Namespace: namespace}, env)).To(Succeed())
			Expect(env.Spec.Sources).To(HaveLen(1))
			Expect(env.Spec.Sources[0].CommitSha).To(Equal("legacy123"))
			Expect(env.Spec.Source).To(BeNil())

			// Cleanup
			k8sClient.Delete(ctx, migrationEnv)
			k8sClient.Delete(ctx, migrationProject)
			targetNsName := "migration-project-migration-env"
			k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: targetNsName}})
		})
	})
})
