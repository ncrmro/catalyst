...
var _ = Describe("Environment Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			teamNs := &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-team-basic-basic",
				},
			}
			// Only create if it doesn't exist (idempotent)
			err := k8sClient.Get(ctx, types.NamespacedName{Name: "test-team-basic-basic"}, &corev1.Namespace{})
			...
			project := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
					Name:      projectName,
					Namespace: "test-team-basic-basic", // Projects are stored in team namespace
				},
			...
			resource := &catalystv1alpha1.Environment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						"catalyst.dev/team":        "test-team-basic-basic",
		...
		AfterEach(func() {
			...
			project := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
					Name:      projectName,
					Namespace: "test-team-basic-basic",
				},
			}
			_ = k8sClient.Delete(ctx, project)

			// Cleanup team namespace
			teamNs := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "test-team-basic-basic"}}
	...
	Context("When reconciling a Helm environment", func() {
		...
		BeforeEach(func() {
			// Use unique namespace name to avoid deletion race conditions
			teamNsName = fmt.Sprintf("test-team-helm-%d", time.Now().UnixNano())
			By("Creating the team namespace: " + teamNsName)
			teamNs := &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name: teamNsName,
				},
			...
			project := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
					Name:      projectName,
					Namespace: teamNsName, // Projects are stored in team namespace
				},
				Spec: catalystv1alpha1.ProjectSpec{
					Sources: []catalystv1alpha1.SourceConfig{
						{
		...
		AfterEach(func() {
			...
			targetNsName := GenerateEnvironmentNamespace(teamNsName, projectName, resourceName)
			_ = k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: targetNsName}})

			// Cleanup team namespace (will be eventually deleted by garbage collection)
			_ = k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: teamNsName}})
		})
		...
		It("should fail when chart path is invalid", func() {
			...
			targetNsName := GenerateEnvironmentNamespace(teamNsName, "invalid-path-project", "invalid-path-env")
			sa := &corev1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "default",
					Namespace: targetNsName,
		...
		It("should fail when SourceRef is missing", func() {
			...
			// Create ServiceAccount to proceed past waiting state
			targetNsName := GenerateEnvironmentNamespace(teamNsName, "missing-ref-project", "missing-ref-env")
			sa := &corev1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "default",
					Namespace: targetNsName,
		...
		It("should fail when helm template is nil", func() {
			...
			// Create ServiceAccount to proceed past waiting state
			targetNsName := GenerateEnvironmentNamespace(teamNsName, "no-template-project", "no-template-env")
			sa := &corev1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "default",
					Namespace: targetNsName,
	...
	Context("When reconciling a Zero-Config environment", func() {
		...
		BeforeEach(func() {
			...
			teamNs := &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-team-build",
				},
			}
			// Only create if it doesn't exist (idempotent)
			err := k8sClient.Get(ctx, types.NamespacedName{Name: "test-team-build"}, &corev1.Namespace{})
			if errors.IsNotFound(err) {
				Expect(k8sClient.Create(ctx, teamNs)).To(Succeed())
			}
			...
			registrySecret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "test-team-build", // Create in team namespace, not environment namespace
				},
				Data: map[string][]byte{
			...
			project := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
					Name:      projectName,
					Namespace: "test-team-build", // Projects are stored in team namespace
				},
				Spec: catalystv1alpha1.ProjectSpec{
					Sources: []catalystv1alpha1.SourceConfig{
						{
			...
			resource := &catalystv1alpha1.Environment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: namespace,
					Labels: map[string]string{
						"catalyst.dev/team":        "test-team-build",
						"catalyst.dev/project":     projectName,
						"catalyst.dev/environment": resourceName,
					},
				},
		...
		AfterEach(func() {
			...
			targetNsName := GenerateEnvironmentNamespace("test-team-build", projectName, resourceName)
			_ = k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: targetNsName}})

			// Cleanup team namespace (will be eventually deleted by garbage collection)
			_ = k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "test-team-build"}})
		})
	...
	Context("When reconciling a Docker Compose environment", func() {
		...
		BeforeEach(func() {
			...
			project := &catalystv1alpha1.Project{
				ObjectMeta: metav1.ObjectMeta{
					Name:      projectName,
					Namespace: "test-team-compose", // Projects are stored in team namespace
				},
				Spec: catalystv1alpha1.ProjectSpec{
					Sources: []catalystv1alpha1.SourceConfig{
						{
			...
			resource := &catalystv1alpha1.Environment{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: namespace,
					Labels: map[string]string{
						"catalyst.dev/team":        "test-team-compose",
						"catalyst.dev/project":     projectName,
						"catalyst.dev/environment": resourceName,
					},
				},
		...
		AfterEach(func() {
			...
			targetNsName := GenerateEnvironmentNamespace("test-team-compose", projectName, resourceName)
			_ = k8sClient.Delete(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: targetNsName}})
		})
