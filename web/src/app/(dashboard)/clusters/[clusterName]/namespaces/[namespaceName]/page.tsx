...
function PodCard({ pod }: PodCardProps) {
  const age = pod.creationTimestamp ? 
    Math.floor((Date.now() - new Date(pod.creationTimestamp).getTime()) / (1000 * 60 * 60 * 24)) : 0;
  ...
  return (
    <div className="border border-outline rounded-lg p-6 bg-surface shadow-sm">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`text-xs px-2 py-1 rounded border ${getStatusColor(pod.status)}`}>
          {pod.status}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {pod.containers.length > 0 && (
          <div>
            <p className="text-sm text-on-surface-variant mb-2">Containers ({pod.containers.length})</p>
                ...
                <div key={index} className="bg-secondary-container rounded p-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-xs px-1 py-0.5 rounded ${
                      container.ready ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            <Link 
              href={`/clusters/${encodeURIComponent(decodedClusterName)}/namespaces`}
          ...
          <p className="mt-2 text-on-surface-variant">
            Resources in the {decodedNamespaceName} namespace on {decodedClusterName}
          </p>
          {cluster.endpoint && (
