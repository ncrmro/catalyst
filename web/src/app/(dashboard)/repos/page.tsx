			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
				{repos.map((repo) => (
					<RepoCard
						key={repo.id}
						repo={repo}
						isConnected={connectionStatus?.[repo.id] || false}
					/>
				))}
			</div>
		</div>
...
export default async function ReposPage() {
		...
		return (
			<div className="space-y-6">
				...
				<div className="bg-error-container border border-outline rounded-lg p-6">
					...
					<h2 className="text-lg font-semibold text-on-error-container mb-2 text-center">
	...
	const totalRepos =
		reposData.user_repos.length +
		Object.values(reposData.org_repos).reduce(
	...
	const connectedRepos = allRepos.filter((repo) => connectionStatus?.[repo.id]);
	const availableRepos = allRepos.filter(
		(repo) => !connectionStatus || !connectionStatus[repo.id],
	);

	// Separate user repos
	const connectedUserRepos = reposData.user_repos.filter(
		(repo) => connectionStatus?.[repo.id],
	);
	const availableUserRepos = reposData.user_repos.filter(
		(repo) => !connectionStatus || !connectionStatus[repo.id],
	);
		...
		<div className="space-y-6">
			...
			<div>
				...
				<p className="text-sm text-on-surface-variant mt-2">
					{totalRepos} repositories across {reposData.organizations.length + 1}{" "}
					accounts
				...
				{!reposData.github_integration_enabled && (
					<div className="mt-2 p-3 bg-surface-variant/20 border border-outline rounded-md">
						<p className="text-sm flex items-center gap-2">
							...
							<span>
								GitHub integration is not currently enabled. Only repositories
			...
			{connectedRepos.length > 0 && (
				<div className="bg-primary-container/10 border border-primary-container rounded-lg p-6 shadow-sm">
						...
						<div className="mb-8">
							...
							<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
								{connectedUserRepos.map((repo) => (
									<RepoCard key={repo.id} repo={repo} isConnected={true} />
								))}
							</div>
						</div>
					)}
					...
					{reposData.organizations.map((org) => {
						const orgRepos = (reposData.org_repos[org.login] || []).filter(
							(repo) => connectionStatus?.[repo.id],
						);
						...
						<div className="mb-8">
							...
							<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
								{availableUserRepos.map((repo) => (
									<RepoCard key={repo.id} repo={repo} isConnected={false} />
								))}
							</div>
						</div>
					)}
					...
					<div>
						<p className="text-on-surface-variant max-w-md mx-auto">
							{reposData.github_integration_enabled
								...
								: "No repositories found in the database. Connect your GitHub account to discover and import repositories."}
						</p>
						<div className="mt-6">
							<Link
								href="/account?highlight=github"
