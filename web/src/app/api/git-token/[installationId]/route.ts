...
async function getInstallationIdForNamespace(
  ...
  try {
    ...
    const teamLabel =
      parsedNamespace.data.metadata?.labels?.["catalyst.dev/team"];
    const projectLabel =
      parsedNamespace.data.metadata?.labels?.["catalyst.dev/project"];
    ...
    try {
      ...
      const githubInstallationId =
        parsedProject.data.spec?.githubInstallationId;
