...
export async function getUserOctokit(userId: string): Promise<Octokit> {
  ...
  const isPATAllowed = process.env.NODE_ENV !== 'production' || GITHUB_CONFIG.ALLOW_PAT_FALLBACK;
  ...
  throw new Error('No GitHub authentication available. Please set GITHUB_PAT or authorize GitHub App.');
}
 ...
 */
export async function fetchPullRequestsFromRepos(octokit: Octokit, repositories: string[]): Promise<PullRequest[]> {
 ...
 */
export async function fetchIssuesFromRepos(octokit: Octokit, repositories: string[]): Promise<Issue[]> {
  ...
  for (const repoFullName of repositories) {
    try {
      ...
      console.log(`  Found ${actualIssues.length} issues in ${repoFullName} (filtered out ${issues.length - actualIssues.length} PRs)`);
      ...
      for (const issue of actualIssues) {
        ...
        if (labels.some(label => label.toLowerCase().includes('bug') || label.toLowerCase().includes('error'))) {
          type = 'bug';
        } else if (labels.some(label => label.toLowerCase().includes('feature') || label.toLowerCase().includes('enhancement'))) {
          type = 'feature';
        } else if (labels.some(label => label.toLowerCase().includes('idea') || label.toLowerCase().includes('discussion'))) {
          type = 'idea';
        }
      ...
      console.warn(`Could not fetch issues for repository ${repoFullName}:`, error);
    }
  }
 ...
 */
export function determinePRPriority(labels: string[]): 'high' | 'medium' | 'low' {
  if (labels.some(label => label.toLowerCase().includes('urgent') || label.toLowerCase().includes('critical'))) {
    return 'high';
  } else if (labels.some(label => label.toLowerCase().includes('minor') || label.toLowerCase().includes('low'))) {
    return 'low';
 ...
 * @returns Array of pull requests with metadata
 */
export async function fetchUserRepositoryPullRequests(octokit: Octokit): Promise<PullRequest[]> {
