 ...
 */
export async function listPodsInNamespace(namespaceName: string, clusterName?: string): Promise<PodInfo[]> {
  try {
    ...
    return response.items.map((pod: any) => {
      ...
      const containers: ContainerInfo[] = pod.spec?.containers?.map((container: any) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const containerStatus = pod.status?.containerStatuses?.find((status: any) => status.name === container.name);
        return {
          name: container.name,
      ...
      const totalRestartCount = containers.reduce((sum, container) => sum + container.restartCount, 0);
