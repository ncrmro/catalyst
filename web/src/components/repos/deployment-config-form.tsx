...
export function DeploymentConfigForm({}: DeploymentConfigFormProps) {
  ...
  const [activeTab, setActiveTab] =
    useState<keyof DeploymentConfig>("production");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);

  const updateEnvironment = (
    env: keyof DeploymentConfig,
    field: keyof DeploymentEnvironment,
    value: string,
  ) => {
    setConfig((prev) => ({
      ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateEnvironment(activeTab, "dockerfilePath", e.target.value)
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateEnvironment(activeTab, "dockerBakeFile", e.target.value)
                }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateEnvironment(
                          activeTab,
                          "deploymentType",
                          e.target.value as "kubernetes" | "helm" | "none",
                        )
                      }
                      className="sr-only"
                    />
            ...
            {currentEnv.deploymentType === "helm" && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateEnvironment(activeTab, "helmChart", e.target.value)
                  }
        ...
        <div className="bg-primary-container border border-outline rounded-lg p-4">
          ...
          <div className="text-xs text-on-primary-container space-y-1">
            ...
            {activeTab === "staging" && (
              <>
                <p>
                  • Staging deployments are triggered on pushes to develop
                  branch
                </p>
      ...
      <div className="border-t border-outline p-6 bg-background">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(["production", "staging", "preview"] as const).map((env) => (
            <div
              key={env}
              className="bg-surface border border-outline rounded-lg p-4"
            >
              <h4 className="font-medium text-on-surface mb-2 capitalize">
                {env}
              </h4>
              <div className="text-xs text-on-surface-variant space-y-1">
                <p>Dockerfile: {config[env].dockerfilePath || "Not set"}</p>
                <p>Deployment: {config[env].deploymentType}</p>
                {config[env].deploymentType === "kubernetes" &&
                  config[env].kubernetesManifest && (
                    <p>Manifest: {config[env].kubernetesManifest}</p>
                  )}
                {config[env].deploymentType === "helm" &&
                  config[env].helmChart && (
