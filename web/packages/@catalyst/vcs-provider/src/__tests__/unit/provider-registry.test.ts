...
import type { VCSProvider, AuthenticatedClient, ConnectionStatus, TokenData, Repository, FileContent, DirectoryEntry, PullRequest, Review, PRComment, Issue, WebhookEvent, CIStatusSummary, Branch } from '../../types';
...
class MockProvider implements VCSProvider {
  ...
  constructor(id: "github" | "gitlab" | "bitbucket" | "azure", name: string) {
    ...
    this.authenticate = vi.fn().mockResolvedValue({ providerId: this.id, raw: {} });
  }
  ...
  getRepository = vi.fn().mockImplementation(async (_client, owner, repo) => ({ id: '1', name: repo, fullName: `${owner}/${repo}`, owner, private: false, defaultBranch: 'main', htmlUrl: '', updatedAt: new Date() }));
  ...
  // Git operations
  createBranch = vi.fn().mockResolvedValue({ name: 'test-branch', sha: 'sha' } as Branch);
  updateFile = vi.fn().mockResolvedValue({ name: 'file', path: 'path', content: 'content', sha: 'sha', htmlUrl: 'url' } as FileContent);

  // CI/CD
  getCIStatus = vi.fn().mockResolvedValue(null);

  verifyWebhookSignature = vi.fn().mockReturnValue(true);
  parseWebhookEvent = vi.fn().mockReturnValue({ type: 'push', sender: 'test' } as WebhookEvent);
}
...
describe('VCS Provider Registry', () => {
  ...
  it('should throw error if default provider is not registered', () => {
    expect(() => registry.getDefault()).toThrow("Default provider 'github' not registered");
  });
  ...
  it('should throw error when setting default to unregistered provider', () => {
    expect(() => registry.setDefault('bitbucket')).toThrow("Provider 'bitbucket' not registered");
  });
});
...
describe('getVCSClient', () => {
  ...
  it('should throw error for non-existent provider', async () => {
    await expect(getVCSClient('user-789', 'bitbucket'))
      .rejects.toThrow("VCS provider 'bitbucket' not found");
  });
});
