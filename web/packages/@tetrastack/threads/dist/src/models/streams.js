...
import { streams, insertStreamSchema, } from '../database/schema/sqlite';
/**
 ...
 * ```
 */
export function createStreamsModel(db) {
    const base = createModelFactory(db, streams, streams.id, insertStreamSchema);
    return {
        ...base,
        /**
         ...
         * Start a new stream.
         */
        async start(threadId, runId, expiresInMs = 5 * 60 * 1000) {
         ...
         */
        async updateSnapshot(id, snapshot, lastEventId) {
            const updateData = {
                snapshot,
                updatedAt: new Date(),
            };
         ...
         */
        async setResumeToken(id, resumeToken) {
        ...
        async canResume(id) {
            ...
            if (stream.status !== 'active')
                return false;
            if (stream.expiresAt && stream.expiresAt < new Date())
                return false;
            return true;
        },
    };
