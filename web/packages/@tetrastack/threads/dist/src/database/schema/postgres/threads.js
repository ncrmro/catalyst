 ...
 */
import { index, jsonb, pgTable, text, timestamp, integer, uuid, } from 'drizzle-orm/pg-core';
...
export const createThreadTables = () => {
    ...
    const threads = pgTable('threads', {
        ...
        index('idx_threads_scope').on(table.projectId, table.scopeType, table.scopeId),
    ...
    const items = pgTable('items', {
        ...
        role: text('role', { enum: ['user', 'assistant', 'system', 'tool'] })
            .notNull()
            .default('user'),
        parts: jsonb('parts').notNull().$type().default([]),
        runId: uuid('run_id'),
        spanId: uuid('span_id'),
