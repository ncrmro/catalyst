...
describe('Pull Request Pod Docker Build Integration', () => {
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.warn('Cleanup failed (this may be expected if tests failed):', error);
    }
  });
  ...
  describe('Service Account with Buildx Permissions', () => {
    it('should create service account with comprehensive RBAC for Docker builds', async () => {
      ...
      const hasDeploymentPermissions = rules.some(rule => 
      ...
      const hasPodPermissions = rules.some(rule => 
      ...
      const hasSecretPermissions = rules.some(rule => 
        rule.apiGroups?.includes('') && 
  ...
  describe('Docker Image Building Integration', () => {
    it('should create and execute PR pod with Docker image building', async () => {
      ...
      const repoUrlEnv = envVars.find(env => env.name === 'REPO_URL');
      expect(repoUrlEnv?.value).toBe('https://github.com/ncrmro/catalyst.git');
      
      const needsBuildEnv = envVars.find(env => env.name === 'NEEDS_BUILD');
      expect(needsBuildEnv?.value).toBe('true');
      
      const shallowCloneEnv = envVars.find(env => env.name === 'SHALLOW_CLONE');
      expect(shallowCloneEnv?.value).toBe('true');
      
      const dockerfileEnv = envVars.find(env => env.name === 'MANIFEST_DOCKERFILE');
    ...
    it('should verify Docker build execution and completion', async () => {
      ...
      while (podStatus !== 'Succeeded' && podStatus !== 'Failed' && attempts < maxAttempts) {
        ...
        if (attempts % 6 === 0) { // Log every 30 seconds
          console.log(`Pod status after ${attempts * 5} seconds: ${podStatus}`);
        }
      }
      ...
      expect(logs).toContain('✓ Found Dockerfile at: /tmp/workspace/web/Dockerfile');
      ...
      expect(logs).toContain('Building image: ghcr.io/ncrmro/catalyst/web:pr-999');
      expect(logs).toContain('building with "k8s-builder" instance using kubernetes driver');

      // Check for build completion or reasonable progress
      const hasDockerBuildProgress = logs.includes('load build definition from Dockerfile') ||
      ...
      if (podStatus === 'Succeeded') {
        ...
        console.warn('Pod failed, but verifying setup stages completed correctly');
        expect(logs).toContain('✓ All tools verified');
        expect(logs).toContain('✓ Buildx kubernetes driver ready');
      } else {
        // Pod is still running after timeout - this is acceptable for slow builds
        console.log('Pod still running after timeout - build may be in progress');
  ...
  describe('Environment Variable Integration', () => {
    it('should handle different SHALLOW_CLONE configurations', async () => {
      ...
      try {
        await cleanupPullRequestPodJob(fullCloneJobName, testNamespace, 'PRIMARY');
      } catch (error) {
    ...
    it('should handle NEEDS_BUILD=false to skip Docker building', async () => {
      ...
      // Cleanup the test job
      try {
        await cleanupPullRequestPodJob(noBuildJobName, testNamespace, 'PRIMARY');
      } catch (error) {
    ...
    it('should handle missing Dockerfile gracefully', async () => {
      ...
      if (pods.items.length > 0) {
        ...
        try {
          ...
          expect(logs).toContain('✗ Dockerfile not found at: /tmp/workspace/nonexistent/Dockerfile');
          expect(logs).toContain('Available files in web directory:');
          
        } catch (error) {
          console.warn('Could not retrieve logs for invalid dockerfile test:', error);
        }
      }

      // Cleanup the test job
      try {
        await cleanupPullRequestPodJob(invalidDockerfileJobName, testNamespace, 'PRIMARY');
      } catch (error) {
