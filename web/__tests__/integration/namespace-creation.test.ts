...
import { generateProjectNamespace, generateTeamNamespace } from '@/lib/namespace-utils';
...
describe('Namespace Creation Integration', () => {
  ...
  const projectNamespace = generateProjectNamespace(testTeamName, testProjectName);
  ...
  it('should create team namespace if it does not exist', async () => {
    ...
    expect(result.labels?.['catalyst.dev/team']).toBe(testTeamName);
  ...
  it('should create project namespace and team namespace if needed', async () => {
    ...
    expect(result.labels).toBeDefined();
    expect(result.labels?.['catalyst.dev/team']).toBe(testTeamName);
    expect(result.labels?.['catalyst.dev/project']).toBe(testProjectName);
  ...
  it('should handle concurrent namespace creation attempts', async () => {
    ...
    const newProjectNamespace = generateProjectNamespace(testTeamName, newProjectName);
