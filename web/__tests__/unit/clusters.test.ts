...
vi.mock("@kubernetes/client-node", () => {
	return {
		KubeConfig: class {
			...
			loadFromEnvVar = vi.fn(async (_envVar) => {
...
describe("Kubernetes Client Environment Variables", () => {
	...
	it("should load kubeconfig from environment variable", async () => {
		...
		const base64Config = Buffer.from(JSON.stringify(mockKubeConfig)).toString(
			"base64",
		);
		...
		await expect(
			kubeConfig.loadFromEnvVar("KUBECONFIG_PRIMARY"),
	...
	it("should handle missing environment variable", async () => {
		...
		await expect(
			kubeConfig.loadFromEnvVar("KUBECONFIG_NONEXISTENT"),
		).rejects.toThrow("Environment variable KUBECONFIG_NONEXISTENT not found");
	});
	...
	it("should handle invalid base64 data", async () => {
		...
		await expect(
			kubeConfig.loadFromEnvVar("KUBECONFIG_INVALID"),
		).rejects.toThrow(/Failed to decode kubeconfig from KUBECONFIG_INVALID/);
	});
	...
	it("should handle invalid JSON data", async () => {
		...
		await expect(
			kubeConfig.loadFromEnvVar("KUBECONFIG_INVALID_JSON"),
		).rejects.toThrow(
	...
	it("getClusters should dynamically discover all KUBECONFIG_* environment variables", async () => {
				...
				clusters: [
					{
						...
						cluster: { server: "https://primary:6443" },
					},
				],
				contexts: [
					{
				...
				clusters: [
					{
						...
						cluster: { server: "https://production:6443" },
					},
				],
				contexts: [
					{
						name: "production-context",
						context: { cluster: "production-cluster", user: "production-user" },
					},
				],
		...
		Object.entries(testConfigs).forEach(([envVar, config]) => {
			process.env[envVar] = Buffer.from(JSON.stringify(config)).toString(
				"base64",
			);
		});
	...
	it("getClusters should return clusters from environment variables", async () => {
		...
		const fooConfig = {
			...
			kind: "Config",
			clusters: [
				{ name: "foo-cluster", cluster: { server: "https://foo:6443" } },
			],
			contexts: [
				{
					name: "foo-context",
					context: { cluster: "foo-cluster", user: "foo-user" },
				},
			],
		...
		process.env.KUBECONFIG_PRIMARY = Buffer.from(
			JSON.stringify(primaryConfig),
		).toString("base64");
		process.env.KUBECONFIG_FOO = Buffer.from(
			JSON.stringify(fooConfig),
		).toString("base64");
