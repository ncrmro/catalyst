...
- **FR-ENV-026**: The Environment CRD MUST use a **curated subset of Kubernetes-native types** for container configuration instead of custom types. The config fields MUST mirror the corresponding fields from `corev1.Container` and `corev1.PodSpec` so that users familiar with Kubernetes Deployment manifests can intuitively configure environments. The curated fields are: `image`, `command`, `args`, `workingDir`, `ports` (`corev1.ContainerPort`), `env` (`corev1.EnvVar`), `resources` (`corev1.ResourceRequirements`), `livenessProbe` (`corev1.Probe`), `readinessProbe` (`corev1.Probe`), `startupProbe` (`corev1.Probe`), and `volumeMounts` (`corev1.VolumeMount`). This design paradigm (K8s-native types over custom abstractions) MUST be documented in the operator's AGENTS.md and spec.md so future contributors understand and maintain it.
- **FR-ENV-027**: The Environment CRD MUST support two deployment paths: (1) **Helm template** — the operator deploys a Helm chart with user-provided `values`, delegating all manifest creation to the chart (supports charts with built-in development/production modes), and (2) **Managed template** — the operator creates Deployment, Service, and Ingress resources directly from the CRD's container configuration fields. The Helm path MUST NOT require container/probe/resource config in the CRD; the managed path MUST read all container config from the CRD with no hardcoded framework-specific defaults.
- **FR-ENV-028**: Managed services (PostgreSQL, Redis, OpenSearch) MUST be modeled as **named service entries** in the CRD, where each entry maps to its own Kubernetes StatefulSet. Each service entry specifies: a `corev1.Container` spec (curated subset: image, env, ports, resources), a `corev1.PersistentVolumeClaimSpec` for storage, and an optional `database` name (postgres-only). The Project CRD defines service defaults at the template level; the Environment CRD supports per-environment overrides. Credentials MUST be generated as Kubernetes Secrets and injected via `envFrom` or `env[].valueFrom.secretKeyRef`, not hardcoded.
- **FR-ENV-029**: The Project CRD MUST support resource configuration per environment type (development/deployment) via `EnvironmentTemplate.Config`. This includes: container `resources` (`corev1.ResourceRequirements`), init container resources, and namespace `ResourceQuota`. The operator MUST read these from the CRD. Hardcoded defaults serve only as absolute fallbacks when no config exists, and the operator MUST log a warning when using fallback values.
...
- **FR-ENV-031**: The Environment CRD MUST support configurable init containers using `corev1.Container` specs (curated subset). Init containers handle setup tasks like package installation and database migration. The operator MUST NOT hardcode any package manager or migration commands — these are specified as the init container's `command` and `args` fields.
