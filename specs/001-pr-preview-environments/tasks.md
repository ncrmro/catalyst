...
- [ ] T010b Verify pullRequestPods schema matches data-model.md specification (run `npm run db:studio` and check: id, pullRequestId, commitSha, namespace, deploymentName, status, publicUrl, branch, imageTag, errorMessage, resourcesAllocated, lastDeployedAt, createdAt, updatedAt, deletedAt fields exist with correct types)
