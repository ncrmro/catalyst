...
**Goal**: Auto-detect task source from current context or accept explicit input
...
- **source_type**: Optional: explicit source type (branch, pr, spec). Auto-detects if not provided.
- **source_ref**: Optional: PR number, spec file path, or branch name. Uses current context if not provided.
...
2. Inform user: "Step 1/8 complete, outputs: tasks/source_context.json"
3. **Continue workflow**: Use Skill tool to invoke `/tasks.parse_tasks`

---

**Reference files**: `.deepwork/jobs/tasks/job.yml`, `.deepwork/jobs/tasks/steps/detect_source.md`